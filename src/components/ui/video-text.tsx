...
export function VideoText({
    ...
    const updateSvgMask = () => {
      ...
      const newSvgMask = `
        <svg xmlns='http://www.w3.org/2000/svg' width='100%' height='100%'>
          <text x='50%' y='50%' font-family='Galactican Regular' font-size='${responsiveFontSize}'
            font-weight='${fontWeight}' text-anchor='${textAnchor}'
            dominant-baseline='${dominantBaseline}'>${content}</text>
        </svg>
        `;
  ...
  return (
    <Component className={cn(`relative size-full font-galactican`, className)}>
      {/* Create a container that masks the video to only show within text */}
      <div
        className="absolute inset-0 flex items-center justify-center font-galactican"
        style={{
      ...
      >
        <video
          className="h-full w-full object-cover font-galactican"
          autoPlay={autoPlay}
          muted={muted}
          loop={loop}
          preload={preload}
      ...
      <span className="sr-only font-galactican">{content}</span>
    </Component>
  );
}
